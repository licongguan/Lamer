...
base_dir = 'F:/project/public_dataset/CITYSCAPES/cityscapes2coco-master/cityscapes/'
mode = 'train'
...
# 随机选择的样本数量 标注
proportion_num = 0.25  # 从剩余的未标注的图片挑选多少比例 去标注
label_num = math.ceil(int(remain) * proportion_num)  # 向上取整
background_label = list(range(-1, 24, 1)) + [29, 30, 34]  # 实例分割排除的类别序号

imgs_path = osp.join(exp_path, style, "random", str(label_num), "images/")  # 标注存放的位置
...
save_random_labeled_txt = osp.join(exp_path, style, "random", str(label_num), "random_labeled.txt")  # 随机选择的样本索引
...
# 将新标注的加入到源数据集 生成coco
init_labeled_img = osp.join(exp_path, style, "suponly", 'spec_train_images/')  # 初始标注数据
...
if not os.path.exists(solo_det_path):
    ...
    os.makedirs(solo_det_path + "whole")  # 所有实例在一起的可视化
    os.makedirs(solo_det_path + "single")  # 一个实例一张图片的可视化
    os.makedirs(solo_det_path + "json")  # 生成的json
    os.makedirs(solo_det_path + "al_info/")  # 每张图像检测到的每个实例的类别，置信度，点击次数

# 检测参数相关
config_file = 'F:/project/SOLO/MyTrain/ALSOLO/experiments/cityscapes/{}/suponly/solov2_light_512_dcn_r50_fpn_8gpu_3x.py'.format(style)
checkpoint_file = 'F:/project/SOLO/MyTrain/ALSOLO/experiments/cityscapes/{}/suponly/epoch_144.pth'.format(style)
det_thr = 0.25  # 检测阈值
...
save_random_AnnCost_path = osp.join(exp_path, style, "random", str(label_num), "随机选择的样本点击次数排序.txt")  # 随机选择的样本点击次数排序
save_random_all_AnnCost_path = osp.join(exp_path, style, "random", str(label_num), "随机选择的样本总点击次数.txt")  # 随机选择的样本总点击次数

# ours 样本选择
ours_path = osp.join(exp_path, style, "ours", str(label_num))
if not os.path.exists(ours_path):
    os.makedirs(ours_path)
remain_img_uc_txt_path = osp.join(ours_path, "ours_剩余未标注样本的不确定性得分.txt")  # solo检测剩余的样本，按照不确定性排名
save_ours_labeled_txt = osp.join(ours_path, "ours_labeled.txt")  # ours选择的样本索引
save_ours_anncost_txt = osp.join(ours_path, "ours_anncost.txt")  # ours选择的样本的点击次数和图片个数

# ours标注
ours_imgs_path = osp.join(ours_path, "images/")  # 标注存放的位置
